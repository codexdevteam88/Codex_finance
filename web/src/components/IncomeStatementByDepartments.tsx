...
import { departments, getDepartmentForAccount } from '../utils/demoData'

// Income Statement by Departments
export default function IncomeStatementByDepartments() {
  ...
  const applyFilter1 = async () => {
    ...
    try {
      ...
      const departmentData = new Map<string, Map<string, {amount: number, name: string}>>()
      
      departmentsData.forEach(dept => {
        departmentData.set(dept.id, new Map())
      })

      data.forEach((record: any) => {
        // Simulate department assignment based on account
        const deptId = getDepartmentForAccount(record.account_code)
        const deptData = departmentData.get(deptId)
        if (deptData) {
          const existing = deptData.get(record.account_code)
          deptData.set(record.account_code, {
      ...
      const newColumns = departmentsData.map(dept => {
        const deptData = departmentData.get(dept.id) || new Map()
        return {
          key: dept.id,
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, marginBottom: 8 }}>Income Statement by Departments</h1>
        <p style={{ margin: 0, color: 'var(--muted)', fontSize: 14 }}>
          Analyze income statement performance by organizational departments
        </p>
      </div>
      ...
      {loading ? (
        ...
        <MultiColumnTable 
          columns={columns} 
          label="Income Statement by Departments" 
          showTotals={true}
        />
      )}
    </div>
